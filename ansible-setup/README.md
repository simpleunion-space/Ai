...
На примере ansible-setup\inventories\example\inventory.yaml:
...
| user    | 30000:30000 | /bin/bash | да       | обычный (с паролем)  | задаётся локально при наличии `password_plaintext` |
...
Для `user` добавьте поле рядом с остальными параметрами аккаунта в
локальном inventory:
...
Новая площадка добавляется как ещё один каталог под `inventories/`.
